...
static NSInteger kBufforRows =
  30;  // Number of rows that are prevent by scroll picker to end

@interface
HSDatePickerViewController ()<UIPickerViewDataSource,
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        // Set minRowIndex and maxRowIndex to -1, so in property getter we will know
        // that to set it to proper values
        ...
        // self.minDate = [NSDate dateWithTimeIntervalSinceNow:60*60*24*-60 +
        // 3*60*60];
        // self.maxDate = [NSDate dateWithTimeIntervalSinceNow:60*60*24*60 -
        // 3*60*60];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    for (NSUInteger i = 0; i < 3; i++) {
        [self.pickerView selectRow:[self defaultRowValueForComponent:i]
                       inComponent:i
                          animated:NO];
    }
    // Before call of this all parameters must be setted
    [self pickerView:self.pickerView
        didSelectRow:[self defaultRowValueForComponent:DayPicker]
         inComponent:DayPicker];
    ...
    self.separator1View.backgroundColor = self.separator2View.backgroundColor =
      self.separator3View.backgroundColor = self.mainColor;

    self.monthAndYearLabel.textColor = self.mainColor;

    [self.monthNextButton setTitleColor:self.mainColor
                               forState:UIControlStateNormal];
    [self.monthNextButton
      setTitleColor:[self.mainColor colorWithAlphaComponent:0.7]
           forState:UIControlStateHighlighted];
    [self.monthNextButton
      setTitleColor:[self.mainColor colorWithAlphaComponent:0.3]
           forState:UIControlStateDisabled];

    [self.monthPreviousButton setTitleColor:self.mainColor
                                   forState:UIControlStateNormal];
    [self.monthPreviousButton
      setTitleColor:[self.mainColor colorWithAlphaComponent:0.7]
           forState:UIControlStateHighlighted];
    [self.monthPreviousButton
      setTitleColor:[self.mainColor colorWithAlphaComponent:0.3]
    ...
    [self.confirmButton setTitle:self.confirmButtonTitle
                        forState:UIControlStateNormal];
    [self.confirmButton setTitleColor:self.mainColor
                             forState:UIControlStateNormal];
    [self.confirmButton setTitleColor:[self.mainColor colorWithAlphaComponent:0.7]
                             forState:UIControlStateHighlighted];
    ...
    [self.backButton setTitleColor:[self.mainColor colorWithAlphaComponent:0.7]
                          forState:UIControlStateHighlighted];

    // Add gesture recognizer to superview...
    UITapGestureRecognizer* gestureRecognizer = [[UITapGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(cancelTapGesture:)];
    ...
    [self setPickerView:self.pickerView
         rowInComponent:HourPicker
         toIntagerValue:[components hour]
           decrementing:NO
               animated:NO];
    [self setPickerView:self.pickerView
...
- (NSString*)confirmButtonTitle
{
    if (!_confirmButtonTitle) {
        _confirmButtonTitle =
          NSLocalizedString(@"Set date", @"HSDatePicker confirm button title");
    }
       ...
       inComponent:(NSInteger)component
{
    if (component == DayPicker) {
        self.monthAndYearLabel.text = [self.monthAndYearLabelDateFormater
          stringFromDate:[self dateForRow:row]];

        // If picker values are to close to bufforValue scroll it back
        if (row < self.minRowIndex) {
            [self.pickerView selectRow:self.minRowIndex
                           inComponent:component
                              animated:YES];
            [self pickerView:self.pickerView
                didSelectRow:self.minRowIndex
                 inComponent:component];
        }
    ...
    NSInteger firstComponentRowValue =
      [pickerView selectedRowInComponent:DayPicker];
    // If picker values are to close to bufforValue scroll it back
...
- (void)setPickerView:(UIPickerView*)pickerView
       rowInComponent:(NSInteger)component
       toIntagerValue:(NSInteger)value
         decrementing:(BOOL)decrementing
             animated:(BOOL)animated
{
    if (decrementing) {
        BOOL valueSetted = NO;
        for (NSInteger i = 0; i < [self realNumberOfRowsInComponent:component];
             i++) {
        ...
        if (!valueSetted && component == MinutePicker) {
            [pickerView selectRow:[pickerView selectedRowInComponent:HourPicker] - 1
                      inComponent:HourPicker
                         animated:animated];
            if ([[self pickerView:pickerView
                   attributedTitleForRow:[pickerView
                                           selectedRowInComponent:HourPicker]
                            forComponent:HourPicker]
                    .string integerValue] == 0) {
                [pickerView selectRow:[pickerView selectedRowInComponent:DayPicker] - 1
                          inComponent:DayPicker
                             animated:animated];
            }
        }
...
- (NSDate*)dateWithSelectedTime
{
    ...
    NSInteger hour =
      [[self pickerView:self.pickerView
         attributedTitleForRow:[self.pickerView
                                 selectedRowInComponent:HourPicker]
                  forComponent:HourPicker]
          .string integerValue];
    NSInteger minute =
      [[self pickerView:self.pickerView
         attributedTitleForRow:[self.pickerView
                                 selectedRowInComponent:MinutePicker]
                  forComponent:MinutePicker]
          .string integerValue];
    return [[NSCalendar currentCalendar] dateBySettingHour:hour
                                                    minute:minute
                                                    second:0
                                                    ofDate:date
                                                   options:0];
...
- (IBAction)confirmDate:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(hsDatePickerPickedDate:)]) {
        BOOL canBeDismiss = [self.delegate hsDatePickerPickedDate:[self dateWithSelectedTime]];
        if (!canBeDismiss) return;
    }
...
- (IBAction)quitPicking:(id)sender
{
    if ([self.delegate
          respondsToSelector:@selector(
                               hsDatePickerWillDismissWithQuitMethod:)]) {
        [self.delegate hsDatePickerWillDismissWithQuitMethod:QuitWithBackButton];
    }
    void (^success)(void) = nil;
    if ([self.delegate
          respondsToSelector:@selector(
                               hsDatePickerDidDismissWithQuitMethod:)]) {
        success = ^{
