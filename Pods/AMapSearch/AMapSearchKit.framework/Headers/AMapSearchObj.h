...
@property (nonatomic, copy)   AMapGeoPoint *origin; //!< 中心点坐标
...
@property (nonatomic, assign) NSInteger count; //!< 返回的POI数目
@property (nonatomic, strong) NSArray<AMapRoutePOI *> *pois; //!< POI结果，AMapRoutePOI 数组
...
@property (nonatomic, assign) BOOL      requireExtension; //!< 是否返回边界坐标，默认NO
@property (nonatomic, assign) BOOL      showBusinessArea; //!< 是否显示商圈信息，默认NO。
...
@property (nonatomic, assign) NSInteger strategy; //!< 驾车导航策略([default = 0]) 0-速度优先（时间）；1-费用优先（不走收费路段的最快道路）；2-距离优先；3-不走快速路；4-结合实时交通（躲避拥堵）；5-多策略（同时使用速度优先、费用优先、距离优先三个策略）；6-不走高速；7-不走高速且避免收费；8-躲避收费和拥堵；9-不走高速且躲避收费和拥堵
...
@property (nonatomic, assign) NSInteger multipath; //!< 是否提供备选步行方案([default = 0])0-只提供一条步行方案; 1-提供备选步行方案(有可能无备选方案)
@end
...
@property (nonatomic, assign) NSInteger strategy; //!< 公交换乘策略([default = 0])0-最快捷模式；1-最经济模式；2-最少换乘模式；3-最少步行模式；4-最舒适模式；5-不乘地铁模式
