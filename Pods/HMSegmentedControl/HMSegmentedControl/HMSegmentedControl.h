...
typedef NSAttributedString * (^HMTitleFormatterBlock)(
...
typedef NS_ENUM(NSInteger, HMSegmentedControlSelectionStyle) {
  HMSegmentedControlSelectionStyleTextWidthStripe, // Indicator width will only
                                                   // be as big as the text
                                                   // width
  HMSegmentedControlSelectionStyleFullWidthStripe, // Indicator width will fill
                                                   // the whole segment
  HMSegmentedControlSelectionStyleBox,  // A rectangle that covers the whole
                                        // segment
  HMSegmentedControlSelectionStyleArrow // An arrow in the middle of the segment
                                        // pointing up or down depending on
...
// Modified: 只有HMSegmentedControlType = TextImages 时有效
typedef NS_ENUM(NSInteger, HMSegmentedControlImageTextPosition) {
  HMSegmentedControlImageTextPositionDefault,
...
/**
 Modified: 指定在 HMSegmentedControlType = TextImages
 状态时，图片和文字的相对位置
 */
@property(nonatomic, assign)
    HMSegmentedControlImageTextPosition segmentedImageTextPosition;
